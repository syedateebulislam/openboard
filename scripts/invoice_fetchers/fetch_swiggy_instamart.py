#!/usr/bin/env python3
"""Standalone Swiggy Instamart invoice fetcher.

Self-contained: connects to Gmail over IMAP, finds Swiggy Instamart order emails,
...
Usage:
    python scripts/invoice_fetchers/fetch_swiggy_instamart.py [--since-days N] [--limit N] [--dry-run]
...
SENDER_EMAIL = "noreply@instamart.in"
SENDER_EMAILS = ["noreply@instamart.in", "noreply@swiggy.in"]
SUBJECT_PREFIX = "Your Instamart order"
CSV_PATH = REPO_ROOT / "data" / "invoices" / "swiggy_instamart.csv"
RAW_DIR = REPO_ROOT / "data" / "invoices" / "raw" / "swiggy_instamart"
...
def parse(text: str, subject: str) -> Dict[str, str]:
    ...
    # Item lines like "1 x Example Product ...  ₹199.00" between Order Items / Order Summary.
    section = text
    ...
    item_lines = re.findall(r"(\d+\s*[xX]\s*[^\n]+?)\s*\n\s*₹", section)
    items = " | ".join(s.strip() for s in item_lines)

    item_bill = normalize_amount(find_first([
        r"Item\s*Bill\s*\n\s*₹\s*([0-9.,]+)",
    ], text))

    handling_fee = normalize_amount(find_first([
        r"Handling\s*Fee\s*\n\s*₹\s*([0-9.,]+)",
    ], text))
...
def run(args) -> Tuple[int, int]:
    ...
    try:
        ...
        uids = search_uids(imap, SENDER_EMAILS, since_str)
        if not uids:
