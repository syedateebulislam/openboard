# OpenBoardCLI Architecture

> Version: 1.7.0  
> Current branch: main  
> Scope: OpenBoardCLI TUI, non-interactive CLI, shared generated dashboard app

## Overview

OpenBoardCLI is an Ink-based terminal application that generates one shared authenticated React dashboard app. The first dashboard creates the app workspace. Later dashboards are added as tabs in the same generated UI.

```mermaid
flowchart TB
    subgraph OB["OpenBoardCLI CLI and TUI (Ink)"]
        T[TUI screens]
        AG[Agent commands]
...
owns individual dashboard components only. OpenBoardCLI owns the application
...
`src/theme.ts` centralizes OpenBoardCLI TUI colors:
...
This applies only to OpenBoardCLI's terminal UI. The generated React dashboard UI is controlled by user prompts and LLM output.
...
OpenAI Codex auth is isolated: OpenBoardCLI runs `codex` with its own codex home
(`~/.openboard/codex-home` by default; override with `OPENBOARD_CODEX_HOME`).
This keeps OpenBoardCLI's ChatGPT/Codex login separate from OpenClaw or manual
...
agent`) can't redirect OpenBoardCLI's codex auth to its own home. Sign in once via
...
If GitHub push succeeds but direct Vercel CLI auth fails, OpenBoardCLI reports that Vercel Git integration may still deploy the pushed commit.

## Invoice Fetchers (per-biller)

A second, **independent** mail-derived data path for users who keep their own
per-biller invoice scripts. OpenBoardCLI never parses invoice mail itself: it drives
...
- **Credentials never touch disk**: `credentialEnv()` passes
  ...
  process's environment. Fetchers written for OpenBoardCLI ≤ 1.9.0 read
...
- **Credential model**: an IMAP **App Password**, which is what the fetchers use.
  ...
  necessarily plaintext (they cannot decrypt); OpenBoardCLI's own copy uses
...
| `OPENBOARD_CODEX_HOME` | Override OpenBoardCLI's isolated codex auth home (default `~/.openboard/codex-home`; ambient `CODEX_HOME` is ignored) |
