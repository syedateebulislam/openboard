/**
 * Board preset definitions for OpenBoard.
 * Each preset provides a domain-specific starting configuration:
 * default LLM prompt, data column hints, and UI metadata.
 */
import { DASHBOARD_PROMPTS } from './dashboardPrompts.js';

export interface BoardPreset {
  id: string;
  name: string;
  icon: string;
  description: string;
  defaultPrompt: string;
  dataHints: string[];
}

export const BOARD_PRESETS: BoardPreset[] = [
  {
    id: 'health',
    name: 'Health',
    icon: '❤️',
    description: 'Apple Health, Fitbit, Garmin data analysis',
    defaultPrompt: DASHBOARD_PROMPTS.health,
    dataHints: ['steps', 'heart_rate', 'sleep_hours', 'calories', 'weight', 'date'],
  },
  {
    id: 'finance',
    name: 'Finance',
    icon: '💰',
    description: 'Bank transactions, spending categories, budget tracking',
    defaultPrompt: DASHBOARD_PROMPTS.finance,
    dataHints: ['date', 'amount', 'category', 'description', 'account', 'type'],
  },
  {
    id: 'grocery',
    name: 'Grocery',
    icon: '🛒',
    description: 'Grocery spending, items, stores, budget tracking',
    defaultPrompt: DASHBOARD_PROMPTS.grocery,
    dataHints: ['date', 'store', 'item', 'price', 'quantity', 'category'],
  },
  {
    id: 'travel',
    name: 'Travel & Rides',
    icon: '✈️',
    description: 'Flights, hotels, cabs, autos — MakeMyTrip, Uber, Rapido exports',
    defaultPrompt: DASHBOARD_PROMPTS.travel,
    dataHints: ['date', 'provider', 'pickup', 'dropoff', 'mode', 'total_paid'],
  },
  {
    id: 'food',
    name: 'Food & Dining',
    icon: '🍔',
    description: 'Food delivery and dining — Swiggy, Zomato, Instamart orders',
    defaultPrompt: DASHBOARD_PROMPTS.food,
    dataHints: ['order_date', 'restaurant', 'items', 'total_paid', 'discount', 'delivery_fee'],
  },
  {
    id: 'shopping',
    name: 'Shopping',
    icon: '🛍️',
    description: 'E-commerce orders — Amazon, Flipkart order history',
    defaultPrompt: DASHBOARD_PROMPTS.shopping,
    dataHints: ['order_date', 'merchant', 'item', 'category', 'amount', 'discount'],
  },
  {
    id: 'subscriptions',
    name: 'Subscriptions',
    icon: '🔁',
    description: 'Recurring SaaS/OTT/app bills — Anthropic, Apple, Netflix invoices',
    defaultPrompt: DASHBOARD_PROMPTS.subscriptions,
    dataHints: ['document_date', 'merchant', 'description', 'total_paid', 'currency', 'invoice_number'],
  },
  {
    id: 'utilities',
    name: 'Utilities & Bills',
    icon: '💡',
    description: 'Telecom, electricity, society/maintenance bills — Airtel, NoBrokerHood',
    defaultPrompt: DASHBOARD_PROMPTS.utilities,
    dataHints: ['bill_date', 'biller', 'description', 'total_paid', 'taxes', 'due_date'],
  },
  {
    id: 'invoices',
    name: 'Invoices & Receipts',
    icon: '🧾',
    description: 'Mixed invoice/receipt ledgers extracted from email across billers',
    defaultPrompt: DASHBOARD_PROMPTS.invoices,
    dataHints: ['document_date', 'biller', 'invoice_number', 'subtotal', 'taxes', 'discount', 'total_paid', 'currency'],
  },
  {
    id: 'custom',
    name: 'Custom',
    icon: '⚙️',
    description: 'Bring your own data and describe what you want',
    defaultPrompt: DASHBOARD_PROMPTS.custom,
    dataHints: [],
  },
];

/**
 * Returns a preset by its id, throwing if not found.
 */
export function getPreset(id: string): BoardPreset {
  const preset = BOARD_PRESETS.find(p => p.id === id);
  if (!preset) throw new Error(`Unknown preset: ${id}`);
  return preset;
}

/**
 * Returns all available presets.
 */
export function getAvailablePresets(): BoardPreset[] {
  return BOARD_PRESETS;
}

/**
 * Converts a raw board name into a URL/directory-safe slug.
 * - Lowercases
 * - Strips non-alphanumeric characters (except hyphens/spaces)
 * - Replaces whitespace runs with a single hyphen
 * - Collapses consecutive hyphens
 * - Trims leading/trailing hyphens
 */
export function sanitizeBoardName(name: string): string {
  const trimmed = name.trim();
  if (!trimmed) throw new Error('Board name is required');
  return trimmed
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, '')
    .replace(/\s+/g, '-')
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '');
}

/**
 * Creates the board config object from a raw user-provided name.
 * `name` is the slug used for directories/repos; `title` is the display label.
 */
export function createBoardConfig(rawName: string): { name: string; title: string } {
  if (!rawName.trim()) throw new Error('Board name is required');
  return {
    name: sanitizeBoardName(rawName),
    title: rawName.trim(),
  };
}
